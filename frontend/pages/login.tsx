...
export default function LoginPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      // TODO: Replace with real API call to backend: POST /api/auth/login
      await new Promise((r) => setTimeout(r, 800));
      setSuccess(true);
      await new Promise((r) => setTimeout(r, 800));
      // Simulate auth cookie so middleware can restrict/protect routes
      if (typeof document !== 'undefined') {
        const days = remember ? 30 : 1;
        const expires = new Date(Date.now() + days * 24 * 60 * 60 * 1000).toUTCString();
        document.cookie = `auth_token=dummy; Path=/; Expires=${expires}; SameSite=Lax`;
      }
  ...
  return (
    <>
        ...
        <div className="login-card">
          ...
          <form className="login-form" id="loginForm" noValidate onSubmit={onSubmit}>
            <div className="method-switch" role="tablist" aria-label="Sign-in method">
              <button type="button" role="tab" aria-selected={method==='email'} className={`pill ${method==='email' ? 'active' : ''}`} onClick={() => setMethod('email')}>Email</button>
              <button type="button" role="tab" aria-selected={method==='phone'} className={`pill ${method==='phone' ? 'active' : ''}`} onClick={() => setMethod('phone')}>Phone</button>
            </div>
          ...
          <div className="social-buttons">
            <button type="button" className="social-btn" onClick={() => alert('Google OAuth coming soon')}> 
          ...
          {success && (
            <div className="success-message" id="successMessage" role="status" aria-live="polite">
      ...
      <style jsx>{`
        ...
        .login-container { min-height: 100vh; display: grid; place-items: center; background: radial-gradient(1200px 400px at 50% -10%, rgba(91,141,239,.12), transparent), linear-gradient(180deg, #0b1020 0%, #0e1530 100%); padding: 24px; }
        .login-card { width: 100%; max-width: 420px; background: rgba(16, 23, 42, 0.9); border: 1px solid #1b2440; color: #e6edf7; border-radius: 16px; padding: 24px; box-shadow: 0 20px 50px rgba(0,0,0,0.35); position: relative; overflow: hidden; }
        .login-header { text-align: center; margin-bottom: 18px; }
        .logo { display: inline-flex; padding: 10px; border-radius: 12px; background: rgba(99, 91, 255, 0.12); box-shadow: inset 0 0 0 1px rgba(99, 91, 255, 0.25); }
        .login-header h1 { margin: 10px 0 6px; font-size: 22px; font-weight: 700; }
        .login-header p { margin: 0; font-size: 14px; color: #9fb3c8; }

        .login-form { display: grid; gap: 14px; }
  .method-switch { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; background: #0f1730; border: 1px solid #1b2440; padding: 6px; border-radius: 12px; }
  .method-switch .pill { background: transparent; color: #9fb3c8; padding: 8px 10px; border-radius: 8px; border: 0; cursor: pointer; }
  .method-switch .pill.active { background: rgba(91,141,239,0.15); color: #e6edf7; box-shadow: inset 0 0 0 1px rgba(91,141,239,0.35); }
        ...
        .input-group input:focus { border-color: #5b8def; box-shadow: 0 0 0 3px rgba(91, 141, 239, 0.25); }
        ...
        .input-group input:focus + label { top: -8px; font-size: 12px; background: #10172a; color: #cfe0ff; }
  ...
  .checkbox-container { display: inline-flex; align-items: center; gap: 8px; user-select: none; cursor: pointer; color: #c7d3e1; position: relative; }
  /* Hide native checkbox but keep it focusable and accessible */
  .checkbox-container input { position: absolute; opacity: 0; width: 0; height: 0; pointer-events: none; }
  /* Custom square box */
  .checkbox-container .checkmark { width: 18px; height: 18px; border-radius: 6px; border: 1px solid #2a3a6a; background: #0b1326; display: inline-flex; align-items: center; justify-content: center; }
  /* Check icon inside the box (hidden by default) */
  .checkbox-container .checkmark svg { opacity: 0; transform: scale(0.8); transition: opacity .15s ease, transform .15s ease; color: #ffffff; }
  /* Checked state: colorize box and show the icon */
  .checkbox-container input:checked + .checkmark { background: #5b8def; border-color: #5b8def; }
  .checkbox-container input:checked + .checkmark svg { opacity: 1; transform: scale(1); }
  /* Keyboard focus ring */
  .checkbox-container input:focus-visible + .checkmark { box-shadow: 0 0 0 3px rgba(91,141,239,0.35); }
  .form-links { display: inline-flex; gap: 12px; }
  .forgot-link { color: #5b8def; text-decoration: none; font-size: 14px; }
        .forgot-link:hover { text-decoration: underline; }

        .submit-btn { width: 100%; display: inline-flex; align-items: center; justify-content: center; gap: 10px; background: #5b8def; color: white; border: none; padding: 12px 16px; border-radius: 12px; font-weight: 700; cursor: pointer; box-shadow: 0 10px 30px rgba(91,141,239,0.35); transition: transform .15s ease, box-shadow .15s ease, opacity .15s ease; }
        .submit-btn:hover { transform: translateY(-1px); box-shadow: 0 14px 34px rgba(91,141,239,0.45); }
